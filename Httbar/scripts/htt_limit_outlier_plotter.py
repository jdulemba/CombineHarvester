...
for key, submit in block_map.iteritems():
	...
	fig.suptitle('%s M(%s) %s Limits and z-scores' % (parity, mass, width))
	    ...
	    ### plot limits
	ax1.scatter(expected_g, expected_cls, color='black', label='Expected')
	ax1.scatter(usable_g, usable_cls, color='green', label='Observed')
	if invalid_inds:
	    ax1.scatter(invalid_g, invalid_cls, color='blue', label='Invalid Points')
	if outliers_inds:
	    ax1.scatter(outliers_g, outliers_cls, color='red', label='Outliers')
	ax1.grid(which='both')
	ax1.axhline(0.5, color='r', linestyle='--')
	ax1.set_xlim(0.0, round(observed_g.max()+0.1,2))
	ax1.set_ylim(0.001, 1.2)
	ax1.set_ylabel('limit')
	ax1.set_yscale('log')
	ax1.legend(loc='lower left',fontsize=8, scatterpoints=1)

	    ### plot z-scores
	ax2.scatter(usable_g, usable_z, color='green', label='Observed')
	if outliers_inds:
	    ax2.scatter(outliers_g, outliers_z, color='red', label='Outliers')
	ax2.grid(which='both')
