...
for page in xrange(n):
    ...
    #set_trace()
    #    # add label for mass and width of boson
    #_, boson, mass, width = args.output.split('_')
    #boson_label = 'm(%s)=%s GeV, #Gamma/m=%s%%' % (boson, mass, name2val(width))
    #plot.DrawCMSLogo(pads[0], 'CMS', '%s, %s' % (args.cms_label, boson_label), 0, 0.25, 0.00, 0.00)
